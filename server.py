...
class Booking(db.Model):
    ...
    seat_row = db.Column(db.String(50), nullable=False)
...
def get_bookings():
    bookings = Booking.query.order_by(Booking.date_created.desc()).all()
    return jsonify([
        {
            "id": b.id,
            "movie_id": b.movie_id,
...
def create_booking():
    ...
    required = ['movie_id', 'title', 'poster_path', 'customer_name', 'contact', 'seat_row', 'seat_number']
    ...
    db.session.add(b)
    db.session.commit()
    return jsonify({"success": True, "message": "Booking confirmed", "booking_id": b.id})
...
if __name__ == '__main__':
    with app.app_context():
        # Drop & recreate only Booking table (fix schema issues)
        Booking.__table__.drop(db.engine, checkfirst=True)
        Booking.__table__.create(db.engine, checkfirst=True)

        # Make sure other tables (User, Watchlist) still exist
        db.create_all()
